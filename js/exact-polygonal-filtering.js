...

{
  ...
  ( async () => {
    ...
    if ( deviceContext ) {
      ...
      const rasterizers = _.range( 0, 4 ).map( i => new phet.alpenglow.FaceRasterizer( {
      ...
      const boxCanvasContext = deviceContext.getCanvasContext( boxCanvas, 'srgb' );
      const bilinearCanvasContext = deviceContext.getCanvasContext( bilinearCanvas, 'srgb' );
